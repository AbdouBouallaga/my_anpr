...
def tiltfix(img):
	...
	cv2.imshow("thresh",cv2.cvtColor(thresh, cv2.COLOR_BGR2RGB))
	...
	if angle != 90.0:
		...
		cv2.imshow("rotated",cv2.cvtColor(rotated, cv2.COLOR_BGR2RGB))
	else:
		rotated = img
	return (rotated)
...
if __name__ == "__main__":
	...
	cv2.imshow("plate",cv2.cvtColor(plate, cv2.COLOR_BGR2RGB))
	plate = tiltfix(plate)
	text = gettext(plate)
	print(text)
